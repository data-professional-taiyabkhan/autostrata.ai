...
export default function ContactForm(){
  ...
  return (
    <form action={endpoint ? undefined : 'mailto:hello@autostrata.ai'} method={endpoint ? 'POST' : 'get'} onSubmit={onSubmit} className="space-y-3 bg-white/5 border border-white/10 rounded-xl p-4 shadow-glow">
      ...
      <button className="inline-flex items-center justify-center border border-white/10 rounded-lg px-4 py-2 font-semibold bg-gradient-to-br from-brand to-[#3fa2ff] text-[#06121f] shadow-glow">Send message</button>
      {status === 'sent' && <p className="text-green-300">Thanks! We’ll get back to you shortly.</p>}
      {status === 'error' && <p className="text-red-300">Something went wrong. Email us at hello@autostrata.ai</p>}
    </form>
  );
}
